...
def test_12_the_manifest_carries_the_structured_rules() -> None:
    ...
    assert names[-2:] == ["modSimContract", "modSimRng"]
...
#
# What follows compiles the STATEMENTS OF modSimRng.bas into Python and runs
# them. It is a transcription of the source, not a VBA interpreter: it proves
# that the algorithm the module WRITES DOWN reproduces the accepted vectors, and
# it fails the moment a locked expression in the .bas is altered, because every
# expression it evaluates is read out of the file at test time.
#
...
def _transcribe() -> dict:
    ...
    code = strip_comments(SIM_RNG_BAS.read_text(encoding="utf-8"))
    types = _parse_types(code)
    ...
    for name, (kind, literal) in _generated_constants().items():
        namespace[name] = _const(name)
    namespace["_python_source"] = "\n\n".join(source)
    namespace["_types"] = types
    namespace["_procs"] = procs
    exec(compile("\n\n".join(source), "<modSimRng.bas transcription>", "exec"), namespace)
    _CACHE["vba"] = namespace
    return namespace
