...
def main(argv: list[str]) -> int:
    ...
    # PROVENANCE. The archive carries no git metadata of its own, so an
    # independent reviewer cannot tell which commit it was cut from. This file is
    # written INTO the archive only - it is not tracked, changes no source byte,
    # and is the one entry that is not a repository blob.
    commit = _git("rev-parse", revision).decode().strip()
    tree = _git("rev-parse", f"{revision}^{{tree}}").decode().strip()
    provenance = (
        "PCCM review package provenance\n"
        "==============================\n"
        f"commit    {commit}\n"
        f"tree      {tree}\n"
        f"subtree   {SUBTREE}/\n"
        f"files     {len(entries)} tracked blobs, byte-identical to the repository\n"
        "\n"
        "Every other entry in this archive is the raw git blob for its path: no\n"
        "line-ending conversion, no smudge filter, no working-tree read. Verify with\n"
        "  git cat-file blob <commit>:pccm/<path>\n"
        "This PROVENANCE.txt is generated by tools/package_review.py and is the only\n"
        "entry that does not correspond to a tracked file.\n"
    )

    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        info = zipfile.ZipInfo(PREFIX + "PROVENANCE.txt")
        info.date_time = (1980, 1, 1, 0, 0, 0)
        info.compress_type = zipfile.ZIP_DEFLATED
        info.external_attr = 0o644 << 16
        archive.writestr(info, provenance)
    ...
    print(f"{output}  ({len(entries)} tracked files from {commit[:7]} ({revision}:{SUBTREE}), plus PROVENANCE.txt)")
    return 0
