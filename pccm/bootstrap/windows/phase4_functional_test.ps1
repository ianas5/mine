...
try {
    ...
    # PORTABLE case authority, and one host-local numerical measurement set. The
    # harness restates neither: addresses come from the projection, the exact
    # expectations and the comparison policy from the authority, and the floating
    # ladder from the measurements.
    $simInspectPath = Join-Path $BuildDir 'phase6_gate_b_inspection.json'
    $simCasesPath = Join-Path $BuildDir 'phase6_gate_b_cases.json'
    # The host-local oracle evidence. Generated by Stage-A on THIS machine, before
    # Excel starts and independent of anything VBA produces; the parity scenario
    # compares against it under the accepted Step-0 evidence policy.
    ...
    foreach ($required in @($manifestPath, $scenarioPath, $casesPath, $inspectPath,
                            $simInspectPath, $simCasesPath, $simOraclePath)) {
  ...
  try {
    ...
    try {
        Invoke-Phase6GateBScenarios -Excel $excel -Workbook $wb -Manifest $manifest `
            ...
            -ArtefactIdentity $phase6Artefacts -OracleEvidence $simOracle
    } catch {
