...
$ git log --format='%h %s' --grep='Phase 10 Step 2B: Reset Results clears every publication and reaches no input' -1
$ git log --format='%h %s' --grep='Phase 10: the runtime protection reconciliation - a structural command window' -1
$ git log --format='%h %s' --grep='Phase 10: runtime year cells are unlocked by the shared paint owner' -1
```
...
| `pccm/src/vba/modSimReport.bas` | Phase 10 Step 2B: Reset Results clears every publication and reaches no input | P10-2B, Reset Results | Adds `SimReportClearPublication` and `SimReportRestorePublication`, the clear/restore pair Reset Results asks this owner for, and `PublicationRecordRange` to address the record it clears. **No derivation, no run, no fingerprint and no existing procedure changed.** |
| `pccm/src/vba/modSimPostReport.bas` | Phase 10 Step 2B: Reset Results clears every publication and reaches no input | P10-2B, Reset Results | Adds `SimPostReportClearPublication` and its restore, which blank the two sensitivity stamp cells and clear the publication blocks after capturing them. **No ranking, no replay and no existing procedure changed.** |
| `pccm/src/vba/modSimAnnualStore.bas` | Phase 10 Step 2B: Reset Results clears every publication and reaches no input | P10-2B, Reset Results | Adds `SimAnnualStoreClearPublication` and its restore on the same terms. **No state rule and no existing procedure changed.** |
| `pccm/src/vba/modCalcReport.bas` | Phase 10 Step 2B: Reset Results clears every publication and reaches no input | P10-2B, Reset Results | Adds `CalcReportClearPublication`, `CalcReportRestorePublication` and the private block helpers they share. **No derivation and no existing procedure changed.** |
| `pccm/src/vba/modAppState.bas` | Phase 10: the runtime protection reconciliation - a structural command window | P10-RP, protection probe runs 5-10 | Windows disproved the accepted assumption that `UserInterfaceOnly` permits ListObject structural mutation on a protected sheet. Owns the declared structural window - opened around a structural mutation, closed on every path, depth-counted so a nested open cannot leave a sheet unprotected. **No business rule, refusal text, ordering or arithmetic changed**; the window is the one thing added, and the Phase-10 record carries the ten probe runs that settled it. |
| `pccm/src/vba/modDrivers.bas` | Phase 10: the runtime protection reconciliation - a structural command window | P10-RP, protection probe runs 5-10 | Windows disproved the accepted assumption that `UserInterfaceOnly` permits ListObject structural mutation on a protected sheet. Add and Delete open the window around their own `ListRows` mutation and close it on every path, including refusal and rollback. **No business rule, refusal text, ordering or arithmetic changed**; the window is the one thing added, and the Phase-10 record carries the ten probe runs that settled it. |
| `pccm/src/vba/modTimeline.bas` | Phase 10: the runtime protection reconciliation - a structural command window | P10-RP, protection probe runs 5-10 | Windows disproved the accepted assumption that `UserInterfaceOnly` permits ListObject structural mutation on a protected sheet. Apply / Update Timeline opens the window around the year-column and profile-row reshaping and closes it on every path, including the restore. **No business rule, refusal text, ordering or arithmetic changed**; the window is the one thing added, and the Phase-10 record carries the ten probe runs that settled it. |
| `pccm/src/vba/modCalcReport.bas` | Phase 10: the runtime protection reconciliation - a structural command window | P10-RP, protection probe runs 5-10 | Windows disproved the accepted assumption that `UserInterfaceOnly` permits ListObject structural mutation on a protected sheet. Publication of the calculation blocks runs under the same window, so the protected `_Calc` sheet receives them the way the probe proved permitted. **No business rule, refusal text, ordering or arithmetic changed**; the window is the one thing added, and the Phase-10 record carries the ten probe runs that settled it. |
| `pccm/src/vba/modWorkbook.bas` | Phase 10: runtime year cells are unlocked by the shared paint owner | P10-R4, Phase 10 final Windows acceptance run 10 | Commit `548799f`. A runtime ListObject year-cell protection defect: the project-year columns of the three grids are materialised at runtime by `ListColumns.Add`, and run 10 read them `Locked=True` with protection on — a keyed year cell from the first Apply Timeline, a regrown keyed cell and a regrown unkeyed reserved cell alike — while the accepted protection projection declares every one of them unlocked. `PaintYearCells`, the shared post-materialisation treatment every profiling and inflation year column already receives, now sets `Locked = False` on every year body cell it traverses, keyed or unkeyed, before its unchanged fill treatment; `SnapshotTable` / `RestoreTable` capture and restore the per-cell `Locked` state beside the fill, so a rollback that rebuilds a column or a row puts back what the user could type in before. Fixed columns and headers stay outside the treatment. **No Phase-7 algorithm, numerical contract, simulation contract or acceptance authority changed**: the module's Phase-7 procedures are byte-identical, the change is mechanically reversible to the pre-P10-R4 bytes through `tests/vba_runtime_lock_state.py`, and Phase 7 remains CLOSED with `79d4c3e` and `ad78988` where they are. |
