...
for i,(wbs,item,*_rest) in enumerate(COST_LINES):
    ...
    tot=put(cp,r,CP_FY0+Y,f"=SUM({L(CP_FY0)}{r}:INDEX({L(CP_FY0)}{r}:{L(CP_FY0+Y-1)}{r},1,{NYEARS_CELL}))",fmt=PCT0,al=C); tot.font=F(bold=True,size=10)
CP_LAST=CPF+K-1
...
for i,rk in enumerate(RISKS):
    ...
    tot=put(rp,r,RP_FY0+Y,f"=SUM({L(RP_FY0)}{r}:INDEX({L(RP_FY0)}{r}:{L(RP_FY0+Y-1)}{r},1,{NYEARS_CELL}))",fmt=PCT0,al=C); tot.font=F(bold=True,size=10)
RP_LAST=RPF+J-1
